...
userRouter.post('/signup',(req,res)=>{
    ...
    return res.status(201).json({message:"Registration successful",data:userDB})
...
userRouter.post('/signin',(req,res)=>{
    try{
        ...
        if(!userFound){
            return res.status(400).json({message:"Incorrect email id and password"})
        }
        ...
        return res.status(200).json({message:"Login successful",user:userFound})
    }
...
userRouter.patch('/:id',(req,res)=>{
    try{
     ...
     let newPassword="567891234"
     if(userFound.password!==newPassword){
        userFound.password=newPassword
     }
     else{
        return res.status(400).json({message:"Same as old password, please try a new one"})
     }
    
    return res.status(200).json({message:"password updated",updated:userDB})
...
userRouter.delete('/:id',(req,res)=>{
    try{
         ...
         return res.status(200).json({message:"User deleted successfully", data:userDB})
    }catch(error){
